...
export const VCreateUser = z.object({
    name: z.preprocess(val => val === "" ? undefined : val, z.string().trim().min(3, "Name must be at least 3 characters")),
    email: z.preprocess(val => val === "" ? undefined : val, z.string().trim().email("Invalid email")
        .nonempty("Email is required")),
    phone: z.preprocess(val => val === "" ? undefined : val, z.string().trim().regex(/^(\+91)?[6-9]\d{9}$/, { message: "Invalid phone number" })
        .nonempty("Phone number is required")),
    password: z.preprocess(val => val === "" ? undefined : val, z.string().trim().min(6, "Password must be at least 6 characters")
