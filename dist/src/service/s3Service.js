...
class S3FileService {
    ...
    initializeMultipartUpload = async (fileKey, fileType) => {
        try {
            const input = {
                Bucket: s3Config.bucket,
                Key: fileKey,
    ...
    async uploadFile(key, fileContent, contentType, bucket = s3Config.bucket) {
        try {
    ...
    async getPreSignedUrl(fileName, type = "put", bucket = this.config.s3_bucket) {
        let key = "";
        key += fileName;
        const params = {
            Bucket: bucket,
    ...
    async deleteFile(key, bucket = s3Config.bucket) {
        try {
